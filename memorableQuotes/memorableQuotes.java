...
public class memorableQuotes {
    ...
    public static void getRandomQuote(ArrayList<String> quotes) {
        Random ran = new Random();
        int n = ran.nextInt(quotes.size());
        // return (quotes.get(n));
        printQuote(quotes.get(n), n, quotes);
    }

    // prints quotes in a new format
    public static void printQuote(String quote, int index, ArrayList<String> quotes) {
        quoteFormatter.incrementCount(quote, index, quotes);
        String[] split = quote.split("@");
        System.out.println(index + ". " + '"' + split[0] + '"' + " (" + split[3] + ")" + "\n -- " + split[1]);
    }
        ...
        for (String i : quotes) {
            ...
            if (type == "content") {
                if (split[0].contains(name)) {
                    printQuote(i, quotes.indexOf(i), quotes);
                    count++;
                }
            }
    ...
    public static void displayRandomQuote(ArrayList<String> quotes, String name) {
        ...
        while (x < max) {
            ...
            // printQuote(quote, quotes.indexOf(quote), quotes);
            ...
            if (memorable.size() != 0) {
                if ("all".equals(parameter)) {
                    for (String i : memorable) {
                        printQuote(i, memorable.indexOf(i), memorable);
                    }
                } else if ("random".equals(parameter)) {
                    getRandomQuote(memorable);
